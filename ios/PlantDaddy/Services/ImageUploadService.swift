...
extension APIEndpoint {
    static func plantUploadURL(id: Int) -> APIEndpoint {
        return .custom("/plants/\(id)/upload-url")
    }

    static func plantCompleteImageUpload(id: Int) -> APIEndpoint {
        return .custom("/plants/\(id)/image")
    }

    // Custom endpoint support
    case custom(String)

    var pathForCustom: String {
        switch self {
...
// Update APIEndpoint to support custom paths
extension APIEndpoint {
    var urlWithCustomSupport: URL? {
        let pathString: String
        switch self {
        case .custom(let customPath):
            pathString = customPath
        default:
            pathString = self.path
        }
        return URL(string: APIConfig.baseURL + pathString)
    }
}
